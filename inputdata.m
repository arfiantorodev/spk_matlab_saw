...
% Last Modified by GUIDE v2.5 25-Oct-2014 16:11:44
...
% Eksekusi Query
sql = exec(dbConn,'SELECT * FROM pemohon');
result = fetch(sql);
...
sql = ['delete from pemohon where Id_Kriteria = ' '''' A ''''];
result = exec(dbConn, sql);
curs = fetch(result);
curs.Data
...
fastinsert(dbConn,'pemohon',{'Telepon','Id_Kriteria','Nama_Pemohon','Usia','JenisKelamin'}, {A,B,C,D,E});
...
if (hObject == handles.radiobutton1)
    set(handles.text12,'String','Laki-laki');
else
    set(handles.text12,'String','Perempuan');
end
